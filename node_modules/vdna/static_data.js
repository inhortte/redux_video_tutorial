...
module.exports = {
  ...
  interestsToDecArr: function(interestsArr, mapping) {
  ...
  tallyExtraInterests: function(interestsArr, mapping) {
    var _mapping = mapping === undefined ? this.arbitraryDecMapping() : mapping;
  ...
  // ---------------------
  decArrToInterests: function(decArr, mapping) {
    var _mapping = mapping === undefined ? this.arbitraryDecMapping() : mapping;
  ...
  _appendDivs: function(vdnaRootName, numero) {
  ...
  gatherVdna: function() {
    ...
    var selectedInterests = this.getSelectedInterests();
    var selectedInterestKeys = Object.keys(selectedInterests);
    console.log("Selected interests: " + JSON.stringify(selectedInterestKeys));
    $("*[vdnaroot]").each(function(index, vdnaRootEl) {
      var vdnaRootName = $(vdnaRootEl).attr("vdnaroot");
      ...
      console.log('originalVdna Count: (' + vdnaRootName + '): ' + that.originalVdnaDivs[vdnaRootName].length);
      that.originalVdnaDivs[vdnaRootName].forEach(function(vdnaClassElStr, index) {
      // $(vdnaRootEl).find("*[vdnaclass]").each(function(index, vdnaClassEl) {
        that.vdnaDivs[vdnaRootName][0].push(vdnaClassElStr);
        ...
        if(show) {
          that.vdnaDivs[vdnaRootName][2].push(vdnaClassElStr);
          if(that.vdnaDivs[vdnaRootName][4].length < that.truncateLength) {
            that.vdnaDivs[vdnaRootName][4].push(vdnaClassElStr);
          }
        }
      });
      // because Array.prototype.sort is DESTRUCTIVE!
      that.vdnaDivs[vdnaRootName][1] = that.vdnaDivs[vdnaRootName][2].reduce(function(memo, el) {
        memo.push(el);
        return memo;
      }, []).sort(that._weightSort);
      that.vdnaDivs[vdnaRootName][3] = that.vdnaDivs[vdnaRootName][4].reduce(function(memo, el) {
        memo.push(el);
        return memo;
      }, []).sort(that._weightSort);
    });
